...
const Contact = () => {
  return (
    ...
    >
      <div className="via-primary/35 pointer-events-none absolute inset-x-0 top-0 h-px bg-linear-to-r from-transparent to-transparent" />
      ...
      <div className="container">
        <div className="relative -mx-4 flex flex-wrap items-center gap-y-12">
          <ScrollReveal className="w-full px-4 lg:w-5/12" variant="fade-right">
            <span className="border-primary/15 text-primary mb-5 inline-flex rounded-md border bg-white/70 px-4 py-2 text-sm font-semibold dark:bg-white/5">
              Kontak
            </span>
            <h2 className="mb-5 max-w-[680px] text-3xl leading-tight font-bold text-black sm:text-4xl lg:text-[42px] dark:text-white">
              <span className="hero-rainbow-text bg-[linear-gradient(90deg,#111827_0%,#8B5CF6_24%,#38BDF8_48%,#22C55E_68%,#EC4899_100%)] [box-decoration-break:clone] bg-clip-text text-transparent [-webkit-box-decoration-break:clone] dark:bg-[linear-gradient(90deg,#FFFFFF_0%,#C4B5FD_24%,#7DD3FC_48%,#86EFAC_68%,#F9A8D4_100%)]">
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              ...
              <a
                ...
                className="border-primary/20 text-dark hover:border-primary hover:text-primary dark:hover:border-primary inline-flex min-h-[56px] items-center justify-center rounded-md border bg-white/70 px-8 text-base font-semibold duration-300 dark:bg-white/5 dark:text-white"
              >
                Kirim Email
              </a>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2 sm:gap-5">
              {contactCards.map((item, index) => (
                <ScrollReveal
                  ...
                  variant="zoom-in"
                >
                  <div className="relative flex flex-col overflow-hidden rounded-md border border-white/70 bg-white/80 p-4 shadow-[0_14px_34px_rgba(88,28,135,0.08)] backdrop-blur-sm sm:min-h-[260px] sm:p-7 sm:shadow-[0_18px_45px_rgba(88,28,135,0.08)] dark:border-white/10 dark:bg-white/5 dark:shadow-none">
                    <span
                      className={`absolute inset-x-0 top-0 h-1 ${item.accent}`}
                    />
                    <div className="mb-3 flex items-center gap-3 sm:mb-6 sm:gap-4">
                      <span
                        className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-md bg-[#EEF1FA] sm:h-14 sm:w-14 ${item.iconColor} dark:bg-white/10`}
                      >
                        {item.icon}
                      </span>
                      <div>
                        <h3 className="text-base leading-snug font-bold text-[#172554] sm:text-lg dark:text-white">
                          {item.title}
                        </h3>
                      </div>
                    </div>
                    <p className="text-body-color dark:text-body-color-dark text-[13px] leading-relaxed font-medium sm:text-sm">
                      {item.text}
                    </p>
                  </div>
                </ScrollReveal>
